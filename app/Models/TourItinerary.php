...
class TourItinerary extends Model
{
    ...
    public function getMealLabelsAttribute(): array
    {
        ...
        $labels = [
            'id' => ['breakfast' => 'Sarapan',      'lunch' => 'Makan Siang', 'dinner' => 'Makan Malam'],
            'ms' => ['breakfast' => 'Sarapan Pagi', 'lunch' => 'Makan Tengah', 'dinner' => 'Makan Malam'],
            'en' => ['breakfast' => 'Breakfast',    'lunch' => 'Lunch',        'dinner' => 'Dinner'],
        ];

        $map = $labels[$locale] ?? $labels['id'];

        $meals = is_array($this->meals_included)
            ? $this->meals_included
            : array_map('trim', explode(',', $this->meals_included));

        return array_map(fn ($meal) => $map[strtolower($meal)] ?? $meal, $meals);
    }
}
