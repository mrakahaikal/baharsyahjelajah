...
<x-layouts::app
    ...
    <section class="border-b border-slate-200 bg-slate-50 py-10 sm:py-12">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <a href="{{ route('transport.show', ['locale' => $locale, 'vehicle' => $slug]) }}" class="inline-flex items-center gap-2 text-sm font-semibold text-blue-600 hover:text-blue-700"><x-lucide-arrow-left class="h-4 w-4" />{{ __('transport.booking.back') }}</a>
    ...
    <section class="bg-slate-50 py-10 sm:py-14">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            @if(filled($whatsappNumber))
                <livewire:vehicle-booking-form :$vehicle :$initialRate :$initialPax />
            @else
