@php
    ...
    $loadingTargets = 'destination,category,type,resetFilters,clearFilter,gotoPage,previousPage,nextPage';
@endphp
...
<div class="-mt-8" x-data="{ filtersOpen: false }" x-on:keydown.escape.window="filtersOpen = false">
    ...
    <section id="tour-results" class="scroll-mt-24 bg-white py-10 sm:py-12" aria-labelledby="tour-results-heading">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="hidden items-end justify-between gap-8 border-b border-slate-200 pb-6 lg:flex">
                <div class="flex min-w-0 flex-1 items-end gap-8">
                    <fieldset class="min-w-0">
                        <legend class="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">{{ __('frontend.tour.index.filters.category') }}</legend>
                        <div class="flex flex-wrap gap-2">
                            <button type="button" wire:click="$set('category', '')" aria-pressed="{{ $category === '' ? 'true' : 'false' }}" class="min-h-11 rounded-full px-4 text-sm font-semibold transition-colors {{ $category === '' ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 hover:text-slate-900' }}">
                                {{ __('frontend.tour.index.filters.all') }}
                            </button>
                            @foreach($categories as $item)
                                @php($isCategoryActive = $category === $item->slug || $category === $item->getTranslation('slug', 'id'))
                                <button type="button" wire:key="desktop-category-{{ $item->id }}" wire:click="$set('category', '{{ $item->slug }}')" aria-pressed="{{ $isCategoryActive ? 'true' : 'false' }}" class="min-h-11 rounded-full px-4 text-sm font-semibold transition-colors {{ $isCategoryActive ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 hover:text-slate-900' }}">
                                    {{ $item->name }} <span class="ml-1 text-xs opacity-70">{{ $item->active_tours_count }}</span>
                                </button>
                            @endforeach
                        </div>
                    </fieldset>
                    ...
                    <fieldset class="shrink-0">
                        <legend class="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">{{ __('frontend.tour.index.filters.type') }}</legend>
                        <div class="flex rounded-lg bg-slate-100 p-1">
                            <button type="button" wire:click="$set('type', '')" aria-pressed="{{ $type === '' ? 'true' : 'false' }}" class="min-h-9 rounded-md px-3 text-xs font-bold transition-colors {{ $type === '' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900' }}">
                                {{ __('frontend.tour.index.filters.all_types') }}
                            </button>
                            @foreach($tourTypes as $tourType)
                                <button type="button" wire:key="desktop-type-{{ $tourType->value }}" wire:click="$set('type', '{{ $tourType->value }}')" aria-pressed="{{ $type === $tourType->value ? 'true' : 'false' }}" class="min-h-9 rounded-md px-3 text-xs font-bold transition-colors {{ $type === $tourType->value ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900' }}">
                ...
                @if($this->activeFilterCount > 0)
                    <button type="button" wire:click="resetFilters" class="inline-flex min-h-11 shrink-0 items-center gap-2 rounded-md px-3 text-sm font-semibold text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900">
