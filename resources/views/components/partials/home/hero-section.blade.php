...
<section class="relative overflow-hidden text-white" aria-labelledby="home-hero-heading">
            ...
            <div class="swiper-wrapper">
                @foreach($bannerSlides as $slide)
                    <div class="swiper-slide">
                        <img src="{{ $imageUrl($slide->image_path ?? null, 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=1800&q=85') }}" alt="" class="h-full w-full object-cover">
                    </div>
                @endforeach
            </div>
    ...
    <div class="relative mx-auto max-w-7xl px-4 pb-36 pt-16 sm:px-6 lg:px-8 lg:pb-44 lg:pt-24">
        <div class="max-w-3xl" data-aos="fade-up">
            ...
            <div class="mt-8 flex flex-col gap-3 sm:flex-row">
                <a href="#search-panel" class="inline-flex items-center justify-center gap-2 rounded-full bg-blue-600 px-7 py-3.5 text-sm font-semibold text-white shadow-lg shadow-blue-600/20 transition-all duration-300 hover:scale-[1.02] hover:bg-blue-700">
                    Mulai Cari Paket
                    <x-lucide-arrow-right class="h-4 w-4" />
                </a>
                <a href="{{ route('umroh.index', ['locale' => $locale]) }}" class="inline-flex items-center justify-center gap-2 rounded-full bg-white/10 px-7 py-3.5 text-sm font-semibold text-white ring-1 ring-white/20 backdrop-blur-md transition-all duration-300 hover:scale-[1.02] hover:bg-white/20">
                    Lihat Paket Umroh
        ...
        <div class="mt-12 grid max-w-3xl grid-cols-3 gap-4" data-aos="fade-up" data-aos-delay="120">
            @foreach([
                ['value' => '35+', 'label' => 'Destinasi'],
                ['value' => '1.200+', 'label' => 'Jamaah & Traveler'],
                ['value' => '24/7', 'label' => 'Bantuan Perjalanan'],
            ] as $stat)
                <div class="rounded-2xl bg-white/5 p-5 ring-1 ring-white/10 backdrop-blur-sm transition-all duration-300 hover:scale-[1.02] hover:bg-white/10 hover:ring-white/20">
